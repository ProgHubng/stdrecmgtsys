	...
	if (isset($_POST['ok']))
	 {
		...
		$error = array();
		$stmt = $db->prepare("SELECT NULL FROM admin WHERE password=:password");
		...
		if ($error_count == 0) 
		{
			$stmt = $db->prepare("UPDATE admin SET password=:password WHERE email=:email");
			$stmt->execute(array('password' => $password, 'email' => $_SESSION['email']));
